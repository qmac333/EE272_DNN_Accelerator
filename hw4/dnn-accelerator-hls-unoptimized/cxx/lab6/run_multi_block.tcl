...
##
#------------------------------------------------------------
# Basic Training: C++: Lab6 Run Multi Block
#------------------------------------------------------------
...
directive set /top::run/readWriteBlock::run/run/main -PIPELINE_STALL_MODE flush
go architect

# Ignore Write before Read Dependencies
directive set /top::run/sharedMem::run/run/else#4:if:io_write(rd_data1)/else#4:if:read_mem(mem:rsc.@) \
                        -IGNORE_DEPENDENCY_FROM {if#5:write_mem(mem:rsc.@) else#5:if:write_mem(mem:rsc.@)}
directive set /top::run/sharedMem::run/run/if#4:io_write(rd_data0)/if#4:read_mem(mem:rsc.@) \
                        -IGNORE_DEPENDENCY_FROM {if#5:write_mem(mem:rsc.@) else#5:if:write_mem(mem:rsc.@)}
go allocate
